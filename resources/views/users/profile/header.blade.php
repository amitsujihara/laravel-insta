<div class="row">
    ...
    <div class="col-4">
        @if($user->avatar)
            <img src="{{ $user->avatar }}" alt="{{ $user->name }}" class="img-thubnail rounded-circle d-block mx-auto avatar-lg">
        @else
                ...
                @else
                    @if($user->isFollowed())
                        <form action="{{ route('follow.destroy' ,$user->id)}}" method="post" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-outlne-secondary btn-sm fw-bold">Following</button>
                        </form>
                    @else
        ...
        <div class="row mb-3">
            <div class="col-auto">
                <a href="{{ route('profile.show', $user->id) }}" class="text-decoration-none text-dark">
                    <!-- Ternary operater condition: true or faulse -->
